...
function winCheck() {
  if (p1Scores.elementScore >= MATCH_WIN) {
    ...
    winner = "Player 2";
    system.innerHTML =
      "Enemy AI has won due to winning once with all three elements!";
    matchLose.play();
  } else if (
    ...
    winner = "Player 2";
    system.innerHTML =
      "Enemy AI has won due to winning thrice with one element!";
    matchLose.play();
  }
}
...
function compareCards(index) {
  ...
  if (elementResult == WIN) {
    ...
    system.innerHTML =
      "You win as " + p1CardElement + " beats " + p2CardElement;
    roundWin.play();
  } else if (elementResult == LOSE) {
    addScore(p2Scores, p2Card);
    system.innerHTML =
      "Enemy AI wins as " + p2CardElement + " beats " + p1CardElement;
    roundLose.play();
  } else if (elementResult == DRAW) {
    if (p1Card.getPower() > p2Card.getPower()) {
      addScore(p1Scores, p1Card);
      system.innerHTML =
        "You win the round due to having a higher power level card!";
      roundWin.play();
    } else if (p1Card.getPower() < p2Card.getPower()) {
      addScore(p2Scores, p2Card);
      system.innerHTML =
        "Enemy AI wins the round due to having a higher power level card!";
      roundLose.play();
    } else if (p1Card.getPower() == p2Card.getPower()) {
